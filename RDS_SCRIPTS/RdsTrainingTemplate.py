...
desired_path = '/rds/general/user/sa7017/home/classifier_2_classes'
...
# root = "/content/drive/MyDrive/IMPERIAL/PhD"
# sys.path.append(root)
# import preprocess
# reload(preprocess)
# from preprocess import *

import classifier_cnn_lstm_2classes
from classifier_cnn_lstm_2classes import build_emg_classifier

folder_path = "/rds/general/user/sa7017/home/classifier_2_classes/SETS"
...
# Filter out baseline class and prepare data for Classifier with 2 classes
# Function to filter out baseline class ([1, 0, 0]) and reshape labels
def filter_and_reshape(X, y):
    """
    Filters out baseline class samples and reshapes labels to have length 2.
    Args:
        X (numpy.ndarray): Features array.
        y (numpy.ndarray): One-hot encoded labels array.
    Returns:
        tuple: Filtered X and reshaped y.
    """
    # Find indices where baseline class ([1, 0, 0]) is not present
    non_baseline_indices = np.where(y[:, 0] == 0)[0]

    # Filter X and y to exclude baseline class
    X_filtered = X[non_baseline_indices]
    y_filtered = y[non_baseline_indices]

    # Remove the first column of y (baseline class) to reshape to length 2
    y_reshaped = y_filtered[:, 1:]

    return X_filtered, y_reshaped

# Process all datasets
X_train, y_train = filter_and_reshape(X_train, y_train)
X_val, y_val = filter_and_reshape(X_val, y_val)
X_test, y_test = filter_and_reshape(X_test, y_test)
...
num_classes = 2       # Baseline, Preparation, Cancellation
...
results_path = '/rds/general/user/sa7017/home/classifier_2_classes/RESULTS'
model_name = 'classifier_cnn_lstm_2_classes'
